...
export default function LoginPage() {
  return (
    <main className="relative min-h-screen overflow-hidden">
      ...
      <FloatingPanel className="right-20 top-1/4 hidden xl:block" />
...
function FloatingPanel({ className }: { className: string }) {
  return (
    <div className={`float-chip pointer-events-none fixed ${className}`}>
      <div className="rounded border border-outline-variant bg-surface-container/75 p-4 opacity-70 shadow-ambient backdrop-blur-sm">
        <div className="mb-2 h-2 w-32 overflow-hidden rounded-full bg-outline-variant/30">
          <div className="h-full w-1/3 animate-pulse bg-primary" />
        </div>
        <div className="h-2 w-20 rounded-full bg-outline-variant/30" />
        <p className="mt-3 font-mono text-[0.5rem] uppercase tracking-[0.12em] text-on-surface-variant">
          LINKING_GLOBAL_NODES...
        </p>
      </div>
    </div>
  );
...
function TerminalIcon({ className }: { className: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <path d="m7 10 2 2-2 2M12 14h4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
