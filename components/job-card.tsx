      ...
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
        ...
        <div className="flex w-full flex-col gap-3 md:w-auto md:items-end">
          ...
          <div className="flex w-full flex-col gap-3 sm:flex-row md:w-auto">
            ...
            {showGenerateEmail && (
              <a href={emailHref} className="generate-email-button">
...
function buildApplyEmailHref(job: Job) {
  const target = isEmail(job.details?.apply_email) ? job.details.apply_email.trim() : "";
  const subject = `Application for ${job.job_title} - Job ID ${job.job_id}`;
  const body = [
    `Hello ${job.company_name} team,`,
    "",
    `I am interested in applying for the ${job.job_title} role listed on Jobgrid.`,
    `Job ID: ${job.job_id}`,
    `Source: ${job.source}`,
    "",
